    ...
    void Update()
    {
        ...
        void SetDistance()
        {
            ...
            for (int step = 1; step < rows * columns; step++)
            {
                ...
                {
                    if (obj && obj.GetComponent<GridInfo>().visited == step - 1)
                    {
        ...
        void SetPath()
        {
            ...
            path.Clear();
            if (gridArray[endX, endY] && gridArray[endX, endY].GetComponent<GridInfo>().visited > 0)
            {
                path.Add(gridArray[x, y]);   
                step = gridArray[x, y].GetComponent<GridInfo>().visited - 1;
            }
            else
            {
        ...
        void Init()
        {
            foreach (GameObject obj in gridArray)
            {
                obj.GetComponent<GridInfo>().visited = -1;
            }
            gridArray[startX, startY].GetComponent<GridInfo>().visited = 0;
        }
        bool TestDirection(int x, int y, int step, int direction)
        {
            ...
            {
                case 4:
                    if (x - 1 > -1 && gridArray[x - 1, y] && gridArray[x - 1, y].GetComponent<GridInfo>().visited == step)
                        return true;
                    else
                        return false;
                case 3:
                    if (y - 1 > -1 && gridArray[x, y - 1] && gridArray[x, y - 1].GetComponent<GridInfo>().visited == step)
                        return true;
                    else
                        return false;
                case 2:
                    if (x + 1 < columns && gridArray[x + 1, y] && gridArray[x + 1, y].GetComponent<GridInfo>().visited == step)
                        return true;
                    else
                        return false;
                case 1:
                    if (y + 1 < rows && gridArray[x, y + 1] && gridArray[x, y + 1].GetComponent<GridInfo>().visited == step)
                        return true;
                    else
                        return false;
        ...
        {
            if (gridArray[x, y])
                gridArray[x, y].GetComponent<GridInfo>().visited = step;
        }
